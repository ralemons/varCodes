% function imgs = directorySSIM_New()
...
% Create path info for moving duplicate images
[dirPath,dupeDir] = fileparts(startDir);
dupeDir = fullfile(dirPath,[dupeDir '_Duplicates']);
if isfolder(dupeDir)
    rmdir(dupeDir,'s'); % Create directory
end
mkdir(dupeDir); % Create directory
...
disp(' ')
disp(['>>>> Size of images on disk: ', num2str(sum([dirProps.bytes])/(1000^2),'%.1f'), ' MB'])
disp(['>>>> This function will take around ', num2str(.36011*N,'%.1f'), ' MB of memory to run']);
disp(['>>>> It will take around ', num2str(0.024*N,'%.0f'), ' seconds to load the images'])
disp(' ')
...
%     if ~isempty(cmap)
%         imgsDisp{1} = ind2rgb(imgsDisp{1},cmap);% %% Find same named images
% dirProps = dir(startDir);
%
% % remove all directories from dir listing
% N = size(dirProps,1);
